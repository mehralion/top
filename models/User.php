...
class User extends AbstractCapitalModel
{
    ...
    protected function fieldMap()
    {
        return array(
            ...
            'naim', 'naim_war', 'naim_war', 'pasbaf', 'runa1', 'runa2', 'runa3', 'is_sn',
        );
    }
    ...
    public static function htmlLoginFromArray($user)
    {
        $align = sprintf('<img src="https://i.oldbk.com/i/align_%s.gif">', $user['align']);
        $klan = '';
        if($user['klan'])
            $klan = sprintf('<img src="https://i.oldbk.com/i/klan/%s.gif">', $user['klan']);
        $login = sprintf('<b>%s</b>', $user['login']);
        $level = sprintf('[%d]', $user['level']);
        $inf = sprintf('<a href="https://oldbk.com/inf.php?%d" target="_blank"><img src="https://i.oldbk.com/i/inf.gif" width="12" height="11" alt="Инф. о %s"></a>', $user['id'], $user['login']);
